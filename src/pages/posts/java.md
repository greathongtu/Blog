...
对象通过16个字节的对象头中8个字节的class地址找到 object.class (类对象)，再通过指针找到本地内存（metaspace）中的 instanceKlass（一个表示类的元数据的结构，它包含了类的所有相关信息，包括字段、方法、常量池等。而 static 对象是和类对象在一起的，也就是堆中，JDK6之前是存在方法区的），这里有 _vtable 虚方法表
